...
void MyTcpSocket::recvMsg()
{
    if (!m_bUpload)
    {
        ...
        uint uiPDULen = 0;
        this->read((char*)&uiPDULen, sizeof(uint));//获得总大小
        ...
        {
        case ENUM_MSG_TYPE_REGIST_REQUEST:  //注册请求消息
        {
            char caName[32] = {'\0'};
            char caPwd[32] = {'\0'};
            strncpy(caName, pdu->caData, 32);
            strncpy(caPwd, pdu->caData+32, 32);
            bool ret = OpeDB::getInstance().handleRegist(caName,caPwd);   //交给数据库处理
            PDU *respdu = mkPDU(0);
        ...
        case ENUM_MSG_TYPE_LOGIN_REQUEST:  //登入请求消息
        {
            char caName[32] = {'\0'};
            char caPwd[32] = {'\0'};
            strncpy(caName, pdu->caData, 32);
            ...
            PDU *respdu = mkPDU(0);
            respdu->uiMsgType = ENUM_MSG_TYPE_LOGIN_RESPOND; //登入回复
            if (ret)
            {
        ...
        case ENUM_MSG_TYPE_ALL_ONLINE_REQUEST:
        {
            ...
            uint uiMsgLen = ret.size()*32;  //存储名字,一个名字占32个字节
        ...
        {
            char caName[32] = {'\0'};
            strcpy(caName, pdu->caData); //拷贝传入的名字
            ...
            if (fileInfo.isDir())
            {
                QDir dir;
                dir.setPath(strPath);
                ret = dir.removeRecursively();  //删除全部，包括子目录
            }
        ...
        case ENUM_MSG_TYPE_RENAME_FILE_REQUEST:
        {
            ...
            QString strOldPath = QString("%1/%2").arg(pPath).arg(caOldName);    //旧名字路径
            QString strNewPath = QString("%1/%2").arg(pPath).arg(caNewName);    //新名字路径
            ...
            if (ret)
            {
                strcpy(respdu->caData, RENAME_FILE_OK); //回复信息放在cdData内
            }
            else
            {
        ...
        case ENUM_MSG_TYPE_DEL_FILE_REQUEST:
        {
            ...
            char *pPath = new char[pdu->uiMsgLen];
            memcpy(pPath, pdu->caMsg, pdu->uiMsgLen);
            QString strPath = QString("%1/%2").arg(pPath).arg(caName);  //将名字和路径拼接成新的路径
            qDebug() << strPath;
        ...
        case ENUM_MSG_TYPE_SHARE_FILE_REQUEST:
        {
            ...
            int size = num*32;
            PDU *respdu = mkPDU(pdu->uiMsgLen-size); //路径所占空间
    ...
    else
    {
        ...
        QByteArray buff = readAll();//接受上传数据
        m_file.write(buff);
        m_iRecved += buff.size();
        if (m_iTotal == m_iRecved)
        {
            m_file.close();//接受文件结束，关闭
            m_bUpload = false;
...
void MyTcpSocket::sendFileToClient()
{
    ...
    while (true)
    {
        ...
        {
            //m_pTimer->stop();
            qDebug() << QString::fromLocal8Bit("发送文件内容给客户端过程中失败");
            m_file.close();
            break;
        }
    }
